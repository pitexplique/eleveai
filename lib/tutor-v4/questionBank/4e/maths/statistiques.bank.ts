...
export const statistiquesBank: TutorBankItemV4[] = [
  ...
  ========================================================================= */
  {
    ...
    generate: () => {
      ...
      const c = randomInt(4, 14);
      const total = a + b + c;
      const pct = Math.round((b / total) * 1000) / 10;
      return {
        text: `Un sondage donne ${a} réponses « jamais », ${b} « parfois » et ${c} « souvent ». Quelle est la fréquence des « parfois », en pourcentage ?`,
        format: "short",
        expected: [String(pct)],
        comparator: "number_equal",
        explanation:
          "Définition : une fréquence est un effectif rapporté au TOTAL des réponses.\n\n" +
          "Méthode : on additionne d'abord tous les effectifs.\n\n" +
          `Calcul : ${a} + ${b} + ${c} = ${total} réponses. Puis ${b} ÷ ${total} = ${pct} %.\n\n` +
          `Conclusion : ⚠️ rapporter ${b} à ${a} ou à ${c} donnerait un rapport entre deux catégories, pas une fréquence.`,
        canvas: statGraphCanvas({
          graphType: "barres",
          data: [
            { label: "jamais", value: a },
            { label: "parfois", value: b, color: "#2563eb" },
            { label: "souvent", value: c },
          ],
  ...
  },
  {
    ...
    generate: () => {
      ...
      const manquant = randomInt(5, 22);
      const total = a + b + manquant;
      return {
        text: `Un tableau annonce ${total} élèves en tout : ${a} en 4e A, ${b} en 4e B, et une classe dont l'effectif a été effacé. Combien d'élèves compte-t-elle ?`,
        format: "short",
        expected: [String(manquant)],
        comparator: "number_equal",
        explanation:
          "Définition : dans un tableau d'effectifs, la somme des lignes redonne toujours le total.\n\n" +
          "Méthode : c'est ce contrôle qui permet de retrouver une valeur effacée.\n\n" +
          `Calcul : ${a} + ${b} = ${a + b}, et ${total} − ${a + b} = ${manquant}.\n\n` +
          "Conclusion : ⭐ ce contrôle par la somme sert aussi à repérer une erreur de saisie — si le total ne tombe pas, une ligne est fausse.",
        canvas: statGraphCanvas({
          graphType: "barres",
          data: [
            { label: "4e A", value: a },
            { label: "4e B", value: b },
            { label: "4e C", value: manquant, color: "#dc2626" },
          ],
          display: { showValues: true, showLabels: true },
          size: { width: 300, height: 190 },
        }),
  ...
  },
  {
    ...
    generate: () => {
      const vals = [randomInt(8, 15), randomInt(16, 26), randomInt(4, 9), randomInt(10, 20)];
      ...
      return {
        text: "Le graphique donne le nombre de passages au CDI. De combien le jour le plus fréquenté dépasse-t-il le moins fréquenté ?",
        format: "qcm",
        choices: makeChoices(String(ecart), [
          String(vals[iMax]),
          String(vals[iMin]),
          String(vals[iMax] + vals[iMin]),
          String(Math.round(vals.reduce((x, y) => x + y, 0) / 4)),
          String(ecart + 1),
        ]),
        expected: [String(ecart)],
        comparator: "mcq_exact",
        explanation:
          "Définition : la question ne demande ni le maximum ni le minimum, mais leur ÉCART.\n\n" +
          "Méthode : on repère les deux barres extrêmes, puis on soustrait.\n\n" +
          `Calcul : ${jours[iMax]} donne ${vals[iMax]} et ${jours[iMin]} donne ${vals[iMin]}, soit ${vals[iMax]} − ${vals[iMin]} = ${ecart}.\n\n` +
          `Conclusion : ⚠️ répondre ${vals[iMax]} serait donner le maximum au lieu de l'écart. La moitié des erreurs de lecture viennent de la question, pas du graphique.`,
        canvas: statGraphCanvas({
          graphType: "barres",
          data: jours.map((j, i) => ({ label: j, value: vals[i] })),
          display: { showValues: true, showLabels: true },
          size: { width: 300, height: 190 },
        }),
      };
