...
import { fractionsBank } from "@/lib/tutor/questionBank/fractions.bank";
import { proportionnaliteBank } from "@/lib/tutor/questionBank/proportionnalite.bank";
import { perimetreBank } from "@/lib/tutor/questionBank/perimetre.bank";
import { airesBank } from "@/lib/tutor/questionBank/aires.bank";
import { anglesBank } from "@/lib/tutor/questionBank/angles.bank";
...
export function buildQuestionFromBank(args: {
  ...
  const notRecentlyUsed = fixedFirst.filter(
    (q) => !args.recentQuestionIds.includes(q.id)
  );
