...
#
# paysage droitier  : python -m manim render -qh --disable_caching manim/scripts/cp/en_letter_a.py LetterARight \
#                       -o eleveai-francais-cp-lettre-a-droitier-paysage --media_dir manim/scripts/cp/media
# paysage gaucher   : python -m manim render -qh --disable_caching manim/scripts/cp/en_letter_a.py LetterALeft \
#                       -o eleveai-francais-cp-lettre-a-gaucher-paysage --media_dir manim/scripts/cp/media
# portrait droitier : python -m manim render -qh --disable_caching -r 1080,1920 manim/scripts/cp/en_letter_a.py LetterAPortraitRight \
#                       -o eleveai-francais-cp-lettre-a-droitier-portrait --media_dir manim/scripts/cp/media
# portrait gaucher  : python -m manim render -qh --disable_caching -r 1080,1920 manim/scripts/cp/en_letter_a.py LetterAPortraitLeft \
...
from lettre_p import lune_dessine as _apple  # noqa: E402
from lettre_f import lune_dessine as _ant  # noqa: E402
...
# ⭐ DESSINS REPRIS DE LA SÉRIE FRANÇAISE : pomme → apple, fourmi → ant.
# Même image, même place : c'est ce qui rend la comparaison fr/gb lisible.
MOTS_EN_A = ["apple", "ant", "anchor", "ambulance", "arrow"]
...
def avion_dessine() -> VGroup:
    """ant — le dessin de « fourmi », repris tel quel."""
    return _ant()


def ami_dessine() -> VGroup:
    """An anchor: the ring, the shank, the crossbar and the two flukes."""
    anneau = Circle(radius=0.09, stroke_color=GREY_B, stroke_width=4)
    anneau.set_fill(opacity=0).move_to(np.array([0.00, 0.48, 0]))
    tige = Line(np.array([0.00, 0.39, 0]), np.array([0.00, -0.30, 0]),
                stroke_color=GREY_B, stroke_width=5)
    traverse = Line(np.array([-0.26, 0.22, 0]), np.array([0.26, 0.22, 0]),
                    stroke_color=GREY_B, stroke_width=4)
    bras = VMobject(stroke_color=GREY_B, stroke_width=5)
    bras.set_fill(opacity=0)
    bras.start_new_path(np.array([-0.40, -0.10, 0]))
    bras.add_cubic_bezier_curve_to(
        np.array([-0.38, -0.44, 0]), np.array([0.38, -0.44, 0]),
        np.array([0.40, -0.10, 0]))
    pointes = VGroup(
        Polygon(np.array([-0.40, -0.10, 0]), np.array([-0.50, -0.24, 0]),
                np.array([-0.28, -0.24, 0]),
                stroke_color=GREY_B, stroke_width=4).set_fill(opacity=0),
        Polygon(np.array([0.40, -0.10, 0]), np.array([0.50, -0.24, 0]),
                np.array([0.28, -0.24, 0]),
                stroke_color=GREY_B, stroke_width=4).set_fill(opacity=0),
    )
    return VGroup(anneau, tige, traverse, bras, pointes)


def animal_dessine() -> VGroup:
    """An ambulance: the box, the cab, the red cross, the light, two wheels."""
    # ⛔ CE MOT REMPLACE « axe », abandonné après DEUX essais : le fer, posé ou
    # chevauchant, lisait toujours « drapeau ». Même motif que la gazelle et le
    # goat — quand un dessin échoue deux fois, on change de mot.
    # ⭐ Ce qui marche à cette taille : les OBJETS À SILHOUETTE FRANCHE.
    caisse = Polygon(
        np.array([-0.14, -0.20, 0]), np.array([0.52, -0.20, 0]),
        np.array([0.52, 0.24, 0]), np.array([-0.14, 0.24, 0]),
        stroke_color=WHITE, stroke_width=5).set_fill(opacity=0)
    cabine = VMobject(stroke_color=WHITE, stroke_width=5)
    cabine.set_fill(opacity=0)
    cabine.start_new_path(np.array([-0.14, -0.20, 0]))
    cabine.add_line_to(np.array([-0.54, -0.20, 0]))
    cabine.add_line_to(np.array([-0.54, 0.02, 0]))
    cabine.add_line_to(np.array([-0.36, 0.16, 0]))
    cabine.add_line_to(np.array([-0.14, 0.16, 0]))
    vitre = Polygon(
        np.array([-0.48, 0.00, 0]), np.array([-0.36, 0.10, 0]),
        np.array([-0.20, 0.10, 0]), np.array([-0.20, 0.00, 0]),
        stroke_color=BLEU_CALCUL, stroke_width=3).set_fill(opacity=0)
    croix = VGroup(
        Line(np.array([0.19, -0.10, 0]), np.array([0.19, 0.14, 0]),
             stroke_color=ROUGE_ERREUR, stroke_width=5),
        Line(np.array([0.07, 0.02, 0]), np.array([0.31, 0.02, 0]),
             stroke_color=ROUGE_ERREUR, stroke_width=5),
    )
    gyrophare = Polygon(
        np.array([0.10, 0.24, 0]), np.array([0.30, 0.24, 0]),
        np.array([0.28, 0.36, 0]), np.array([0.12, 0.36, 0]),
        stroke_color=ROUGE_ERREUR, stroke_width=4).set_fill(opacity=0)
    roues = VGroup(
        *[Circle(radius=0.11, stroke_color=GREY_B, stroke_width=4)
          .set_fill(opacity=0).move_to(np.array([x, -0.28, 0]))
          for x in (-0.32, 0.32)]
    )
    return VGroup(roues, cabine, caisse, vitre, croix, gyrophare)

def abricot_dessine() -> VGroup:
    """An arrow: the shaft, the head, and two straight fletchings."""
    # ⛔ L'empennage était fait de TRIANGLES qui chevauchaient la hampe : ça
    # faisait un nœud. Deux traits obliques suffisent, et se lisent.
    hampe = Line(np.array([-0.44, -0.32, 0]), np.array([0.30, 0.28, 0]),
                 stroke_color=ORANGE_RETENUE, stroke_width=5)
    pointe = Polygon(np.array([0.52, 0.46, 0]), np.array([0.20, 0.36, 0]),
                     np.array([0.36, 0.10, 0]),
                     stroke_color=GREY_B, stroke_width=5).set_fill(opacity=0)
    plumes = VGroup(
        *[Line(np.array([-0.44 + k * 0.13, -0.32 + k * 0.11, 0]),
               np.array([-0.52 + k * 0.13, -0.06 + k * 0.11, 0]),
               stroke_color=ROUGE_ERREUR, stroke_width=4) for k in range(3)]
    )
    return VGroup(plumes, hampe, pointe)

def arbre_dessine() -> VGroup:
    """apple — le dessin de « pomme », repris tel quel."""
    return _apple()
...
VOIX = Path(__file__).resolve().parents[3] / "public" / "sons" / "en-lettre-a"
DUREE = { "00-aujourdhui": 3.62, "01-ecoute": 3.77, "02-regarde": 2.96,
    "03-depart": 14.97, "04-encore": 4.15, "05-a-comme": 1.75,
    "05-cherchons": 4.37, "06-arbre": 1.62, "07-avion": 1.67, "08-ami": 1.66,
    "09-animal": 2.04, "10-abricot": 1.58, "10-pareil": 5.16,
    "11-relance": 3.45, "12-va-sur": 3.56, "13-tout": 9.71,
    "14-bientot": 2.23,
}
CLIPS_MOTS = ["06-arbre", "07-avion", "08-ami", "09-animal", "10-abricot"]


class _LetterABase(Scene):
    ...
    def construct(self):
        ...
        titre_mots = Text("a as in…", font_size=50)
...
class LetterARight(_LetterABase):
    """16:9, droitier — YouTube."""


class LetterALeft(_LetterABase):
    """16:9, gaucher — YouTube."""

    gaucher = True


class LetterAPortraitRight(_Portrait, _LetterABase):
    """9:16, droitier."""


class LetterAPortraitLeft(_Portrait, _LetterABase):
    """9:16, gaucher."""
