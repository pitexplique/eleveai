# Writing the cursive letter « q » — CP (English)
#
...
# ⭐ « quiche » et « quarter » GARDENT LEUR DESSIN FRANÇAIS : la quiche est le
# même mot, et le quartier d'orange EST un quarter. Même image, même ordre.
# ⚠️ L'anglais a le même problème que le français — le « q » ne va jamais sans
# son « u » —, et la voix le dit aussi : « the q never goes out on its own ».
# ⛔ Écarté : « quill », la plume, qui porte déjà le dessin de la vidéo du « f ».
# Deux lettres ne doivent pas partager la même image.
MOTS_EN_Q = ["queen", "quilt", "quiz", "quiche", "quarter"]


# ─── Les cinq dessins ─────────────────────────────────────────────────────────
def lune_dessine() -> VGroup:
    """A queen: her crown — three points, a band, and three jewels."""
    couronne = VMobject(stroke_color=JAUNE_TITRE, stroke_width=5)
    couronne.set_fill(opacity=0)
    couronne.start_new_path(np.array([-0.40, -0.30, 0]))
    couronne.add_line_to(np.array([-0.34, 0.14, 0]))
    couronne.add_line_to(np.array([-0.20, -0.06, 0]))
    couronne.add_line_to(np.array([0.00, 0.30, 0]))
    couronne.add_line_to(np.array([0.20, -0.06, 0]))
    couronne.add_line_to(np.array([0.34, 0.14, 0]))
    couronne.add_line_to(np.array([0.40, -0.30, 0]))
    couronne.add_line_to(np.array([-0.40, -0.30, 0]))
    bandeau = Line(np.array([-0.39, -0.18, 0]), np.array([0.39, -0.18, 0]),
                   stroke_color=JAUNE_TITRE, stroke_width=3)
    pierres = VGroup(
        *[Dot(np.array([x, y, 0]), radius=0.045, color=ROUGE_ERREUR)
          for x, y in ((-0.34, 0.18), (0.00, 0.36), (0.34, 0.18))]
    )
    return VGroup(couronne, bandeau, pierres)


def lit_dessine() -> VGroup:
    """A quilt: nine patches, and the stitching between them."""
    cote = 0.60
    cadre = Polygon(
        np.array([-cote / 2, -cote / 2, 0]), np.array([cote / 2, -cote / 2, 0]),
        np.array([cote / 2, cote / 2, 0]), np.array([-cote / 2, cote / 2, 0]),
        stroke_color=ROUGE_ERREUR, stroke_width=5).set_fill(opacity=0)
    grille = VGroup(
        *[Line(np.array([x, -cote / 2, 0]), np.array([x, cote / 2, 0]),
               stroke_color=ROUGE_ERREUR, stroke_width=3)
          for x in (-cote / 6, cote / 6)],
        *[Line(np.array([-cote / 2, y, 0]), np.array([cote / 2, y, 0]),
               stroke_color=ROUGE_ERREUR, stroke_width=3)
          for y in (-cote / 6, cote / 6)],
    )
    # Trois carreaux marques d une croix : c est ce qui dit « patchwork »
    # plutot que « grille » ou « fenetre ».
    motifs = VGroup(
        *[VGroup(
            Line(np.array([x - 0.06, y - 0.06, 0]), np.array([x + 0.06, y + 0.06, 0]),
                 stroke_color=BLEU_CALCUL, stroke_width=3),
            Line(np.array([x - 0.06, y + 0.06, 0]), np.array([x + 0.06, y - 0.06, 0]),
                 stroke_color=BLEU_CALCUL, stroke_width=3))
          for x, y in ((-0.20, 0.20), (0.00, 0.00), (0.20, -0.20))]
    )
    franges = VGroup(
        *[Line(np.array([x, -cote / 2, 0]), np.array([x, -cote / 2 - 0.09, 0]),
               stroke_color=ROUGE_ERREUR, stroke_width=2)
          for x in np.linspace(-0.24, 0.24, 5)]
    )
    return VGroup(cadre, grille, motifs, franges)


def livre_dessine() -> VGroup:
    """A quiz: a big question mark inside a speech bubble."""
    bulle = VMobject(stroke_color=BLEU_CALCUL, stroke_width=5)
    bulle.set_fill(opacity=0)
    bulle.start_new_path(np.array([-0.36, -0.14, 0]))
    bulle.add_cubic_bezier_curve_to(
        np.array([-0.44, 0.34, 0]), np.array([0.44, 0.34, 0]),
        np.array([0.36, -0.14, 0]))
    bulle.add_cubic_bezier_curve_to(
        np.array([0.30, -0.34, 0]), np.array([0.06, -0.34, 0]),
        np.array([-0.06, -0.32, 0]))
    bulle.add_line_to(np.array([-0.20, -0.50, 0]))     # la pointe de la bulle
    bulle.add_line_to(np.array([-0.16, -0.30, 0]))
    bulle.add_cubic_bezier_curve_to(
        np.array([-0.26, -0.28, 0]), np.array([-0.34, -0.22, 0]),
        np.array([-0.36, -0.14, 0]))
    crochet = VMobject(stroke_color=JAUNE_TITRE, stroke_width=5)
    crochet.set_fill(opacity=0)
    crochet.start_new_path(np.array([-0.11, 0.20, 0]))
    crochet.add_cubic_bezier_curve_to(
        np.array([-0.09, 0.32, 0]), np.array([0.13, 0.32, 0]),
        np.array([0.11, 0.18, 0]))
    crochet.add_cubic_bezier_curve_to(
        np.array([0.09, 0.06, 0]), np.array([0.00, 0.06, 0]),
        np.array([0.00, -0.06, 0]))
    point = Dot(np.array([0.00, -0.18, 0]), radius=0.045, color=JAUNE_TITRE)
    return VGroup(bulle, crochet, point)
...
VOIX = Path(__file__).resolve().parents[3] / "public" / "sons" / "en-lettre-q"
DUREE = { "00-aujourdhui": 3.81, "01-ecoute": 4.06, "02-regarde": 3.15,
    "03-depart": 33.12, "04-encore": 4.15, "05-cherchons": 10.89,
    "05-q-comme": 2.48, "06-quatre": 1.67, "07-queue": 1.81,
    "08-quille": 1.68, "09-quiche": 1.76, "10-pareil": 5.16,
    "10-quartier": 1.94, "11-relance": 4.07, "12-va-sur": 3.56,
    "14-bientot": 2.23,
}
...
class _LetterQBase(Scene):
    ...
    def construct(self):
        ...
        titre_mots = Text("q u as in…", font_size=50)
...
class LetterQRight(_LetterQBase):
    """16:9, droitier."""


class LetterQLeft(_LetterQBase):
    """16:9, gaucher."""

    gaucher = True


class LetterQPortraitRight(Portrait, _LetterQBase):
    """9:16, droitier."""


class LetterQPortraitLeft(Portrait, _LetterQBase):
    """9:16, gaucher."""
