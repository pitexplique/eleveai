...
#
# ⭐⭐ LE SECOND CHIFFRE QUI LÈVE LE CRAYON, ET IL LÈVE VERS LE HAUT.
...
#
# portrait droitier : python -m manim render -qh --disable_caching -r 1080,1920 manim/scripts/cp/chiffre_4.py Chiffre7CpPortrait \
#                       -o eleveai-maths-cp-chiffre-4-droitier-portrait --media_dir manim/scripts/cp/media
# portrait gaucher  : python -m manim render -qh --disable_caching -r 1080,1920 manim/scripts/cp/chiffre_4.py Chiffre7CpPortraitGaucher \
...
# ─── Le chemin du « 7 », en DEUX tracés ───────────────────────────────────────
# ⭐ Forme A, choisie sur trois rendues côte à côte : la jambe descend FRANCHEMENT
# vers la gauche, comme le 7 scolaire français. La variante à jambe verticale
# ressemblait à un « 1 » barré.
# ⭐⭐ LA BARRE DU MILIEU N'EST PAS UN ORNEMENT. Elle distingue le 7 du 1 dans
# une écriture d'enfant, où le 1 porte souvent une attaque oblique. C'est une
# convention française et belge — ⚠️ les États-Unis et le Royaume-Uni écrivent
# le 7 SANS barre : la version anglaise devra la retirer, sinon elle enseigne
# un chiffre que le lecteur ne reconnait pas.
TRAIT_1 = [
    np.array([-0.36, 1.92, 0]),   # départ, en haut à gauche
    np.array([0.42, 1.92, 0]),    # la barre du haut, vers la droite
    np.array([-0.06, 0.00, 0]),   # la jambe, en biais vers le bas à gauche
]
# ⛔ LA BARRE DOIT TRAVERSER, ET C'EST UN CALCUL, PAS UN COUP D'ŒIL.
# Première version : de −0,30 à 0,16. Or à la hauteur y = 0,90, la jambe est
# exactement en x = 0,165 — la barre S'ARRÊTAIT DESSUS et faisait un moignon à
# gauche. Un « 7 » barré est barré DES DEUX CÔTÉS ; sinon ce n'est pas une
# barre, c'est un accident.
# ⚠️ La position de la jambe se calcule : elle va de (0,42 ; 1,92) à
# (−0,06 ; 0,00), donc à y = 0,90 elle est à 0,42 + (1 − 0,90/1,92) × (−0,48).
# On déborde ensuite de 0,30 de chaque côté.
_X_JAMBE = 0.42 + (1 - 0.90 / 1.92) * (-0.48)
TRAIT_2 = [
    np.array([_X_JAMBE - 0.30, 0.90, 0]),   # on repose à gauche de la jambe…
    np.array([_X_JAMBE + 0.30, 0.90, 0]),   # …et on traverse jusqu'à droite
]
...
def chemin_7(stroke_width: float = 10, color: str = WHITE) -> VGroup:
...
VOIX = Path(__file__).resolve().parents[3] / "public" / "sons" / "cp-chiffre-7"
DUREE = {
    "00-aujourdhui": 4.30, "01-ecoute": 2.57, "02-regarde": 4.02, "03-depart": 18.65,
...
class _Chiffre7Base(Scene):
    ...
    def construct(self):
        ...
        garde, garde_main = page_de_garde(
            ...
            self, CHIFFRE, chemin_7(stroke_width=12), MascotteMargouillat(),
        ...
        traits = chemin_7(stroke_width=14 if not self.vertical else 16)
        VGroup(lignes, traits).move_to(ORIGIN).shift(DOWN * 0.3)

        modele = chemin_7(stroke_width=14 if not self.vertical else 16, color=GREY_D)
        ...
        for duree in (2.0, 1.2):
            if refait is not None:
                ...
                self.remove(*refait)
            refait = chemin_7(stroke_width=14 if not self.vertical else 16)
...
class Chiffre7Cp(_Chiffre7Base):
    """16:9, droitier — conservée, mais on ne la rend plus."""


class Chiffre7CpGaucher(_Chiffre7Base):
    gaucher = True


class Chiffre7CpPortrait(Portrait, _Chiffre7Base):
    """9:16, droitier — LE format qui est vu."""


class Chiffre7CpPortraitGaucher(Portrait, _Chiffre7Base):
    gaucher = True
