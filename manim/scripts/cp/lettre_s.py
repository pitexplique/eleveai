# Écriture de la lettre « s » en cursive — CP
#
...
# ⭐ CINQ MOTS OÙ LE « s » EST EN INITIALE et se prononce [s], tous dessinables
# en silhouette fermée.
# ⛔ ÉCARTÉS, et c'est le piège de cette lettre-là : les mots où le « s » se dit
# [z] entre deux voyelles (« maison »), et surtout ceux qui commencent par un
# GROUPE — « stylo », « sport » : l'élève y entend [st], pas [s] seul.
MOTS_EN_S = ["soleil", "souris", "serpent", "sac", "salade"]
...
def lit_dessine() -> VGroup:
    """Une souris : le corps, l'oreille, le museau, la queue."""
    corps = Ellipse(width=0.70, height=0.42, stroke_color=GREY_B, stroke_width=5)
    corps.set_fill(opacity=0).move_to(np.array([-0.04, -0.06, 0]))
    # ⚠️ L'oreille est POSÉE SUR le dos, pas dedans : à r = 0,15 centrée en
    # 0,24 elle coupait l'ellipse et traçait une barre en travers du corps.
    oreille = Circle(radius=0.14, stroke_color=GREY_B, stroke_width=4)
    oreille.set_fill(opacity=0).move_to(np.array([-0.10, 0.29, 0]))
    museau = Polygon(np.array([0.30, 0.04, 0]), np.array([0.58, -0.08, 0]),
                     np.array([0.30, -0.16, 0]),
                     stroke_color=GREY_B, stroke_width=4).set_fill(opacity=0)
    truffe = Dot(np.array([0.58, -0.08, 0]), radius=0.04, color=ROUGE_ERREUR)
    oeil = Dot(np.array([0.22, 0.06, 0]), radius=0.03)
    queue = ArcBetweenPoints(np.array([-0.38, -0.10, 0]), np.array([-0.72, 0.22, 0]),
                             angle=-1.8, stroke_color=GREY_B, stroke_width=4)
    return VGroup(queue, corps, oreille, museau, truffe, oeil)
...
def lapin_dessine() -> VGroup:
    """Un sac : le corps, l'anse, le rabat."""
    corps = Polygon(
        np.array([-0.34, -0.44, 0]), np.array([0.34, -0.44, 0]),
        np.array([0.34, 0.12, 0]), np.array([-0.34, 0.12, 0]),
        stroke_color=ORANGE_RETENUE, stroke_width=5).set_fill(opacity=0)
    rabat = Line(np.array([-0.34, -0.10, 0]), np.array([0.34, -0.10, 0]),
                 stroke_color=ORANGE_RETENUE, stroke_width=4)
    fermoir = Dot(np.array([0.00, -0.10, 0]), radius=0.05, color=JAUNE_TITRE)
    anse = ArcBetweenPoints(np.array([-0.20, 0.12, 0]), np.array([0.20, 0.12, 0]),
                            angle=-2.4, stroke_color=ORANGE_RETENUE, stroke_width=5)
    return VGroup(anse, corps, rabat, fermoir)
...
VOIX = Path(__file__).resolve().parents[3] / "public" / "sons" / "cp-lettre-s"
DUREE = {
    "00-aujourdhui": 3.38, "01-ecoute": 3.20, "02-regarde": 3.23, "03-depart": 16.98,
    "04-encore": 2.92, "05-cherchons": 4.32, "05-s-comme": 1.68, "06-soleil": 1.68,
    "07-souris": 1.53, "08-serpent": 1.62, "09-sac": 1.56, "10-salade": 1.66,
    "10-pareil": 4.71, "11-relance": 3.14, "12-va-sur": 3.16, "14-bientot": 1.71,
}
CLIPS_MOTS = ["06-soleil", "07-souris", "08-serpent", "09-sac", "10-salade"]


class _LettreSBase(Scene):
    ...
    def construct(self):
        ...
        titre_mots = Text("s comme…", font_size=50)
        ...
        relance = ecran_relance(self.vertical, "s")
...
class LettreSCp(_LettreSBase):
    """16:9, droitier."""


class LettreSCpGaucher(_LettreSBase):
    """16:9, gaucher."""

    gaucher = True


class LettreSCpPortrait(Portrait, _LettreSBase):
    """9:16, droitier."""


class LettreSCpPortraitGaucher(Portrait, _LettreSBase):
    """9:16, gaucher."""
