# Writing the cursive letter « k » — CP (English)
#
...
# ⭐ QUATRE MOTS SUR CINQ GARDENT LEUR DESSIN FRANÇAIS — c'est le meilleur
# recouvrement de toute la série : kangourou, koala, kiwi et kayak s'écrivent
# presque pareil dans les deux langues. Même image, même ordre : la comparaison
# fr/gb est ici parfaitement propre.
# ⚠️ Seul le « képi » n'a pas d'équivalent : un couple neuf, le CERF-VOLANT.
# ⛔ Écarté : « key », qui porte déjà le dessin de la clé dans la vidéo du « c ».
MOTS_EN_K = ["kangaroo", "koala", "kiwi", "kayak", "kite"]
...
def lampe_dessine() -> VGroup:
    """A kite: the diamond, its two spars, and the tail with three bows."""
    losange = Polygon(
        np.array([0.00, 0.56, 0]), np.array([-0.30, 0.16, 0]),
        np.array([0.00, -0.24, 0]), np.array([0.30, 0.16, 0]),
        stroke_color=ROUGE_ERREUR, stroke_width=5).set_fill(opacity=0)
    barres = VGroup(
        Line(np.array([0.00, 0.56, 0]), np.array([0.00, -0.24, 0]),
             stroke_color=ROUGE_ERREUR, stroke_width=2),
        Line(np.array([-0.30, 0.16, 0]), np.array([0.30, 0.16, 0]),
             stroke_color=ROUGE_ERREUR, stroke_width=2),
    )
    ficelle = VMobject(stroke_color=JAUNE_TITRE, stroke_width=3)
    ficelle.set_fill(opacity=0)
    ficelle.start_new_path(np.array([0.00, -0.24, 0]))
    ficelle.add_cubic_bezier_curve_to(
        np.array([0.16, -0.40, 0]), np.array([-0.16, -0.50, 0]),
        np.array([0.02, -0.66, 0]))
    noeuds = VGroup(
        *[Line(np.array([x - 0.07, y + 0.05, 0]), np.array([x + 0.07, y - 0.05, 0]),
               stroke_color=JAUNE_TITRE, stroke_width=3)
          for x, y in ((0.09, -0.36), (-0.06, -0.50), (0.02, -0.64))]
    )
    return VGroup(ficelle, noeuds, losange, barres)
...
VOIX = Path(__file__).resolve().parents[3] / "public" / "sons" / "en-lettre-k"
DUREE = { "00-aujourdhui": 3.7, "01-ecoute": 4.02, "02-regarde": 3.04,
    "03-depart": 32.62, "04-encore": 4.15, "05-cherchons": 4.54,
    "05-k-comme": 2.09, "06-kangourou": 1.97, "07-koala": 1.82,
    "08-kiwi": 1.79, "09-kayak": 1.95, "10-kepi": 1.8, "10-pareil": 5.16,
    "11-relance": 3.63, "12-va-sur": 3.56, "14-bientot": 2.23,
}
CLIPS_MOTS = ["06-kangourou", "07-koala", "08-kiwi", "09-kayak", "10-kepi"]


class _LetterKBase(Scene):
    ...
    def construct(self):
        ...
        titre_mots = Text("k as in…", font_size=50)
...
class LetterKRight(_LetterKBase):
    """16:9, droitier."""


class LetterKLeft(_LetterKBase):
    """16:9, gaucher."""

    gaucher = True


class LetterKPortraitRight(Portrait, _LetterKBase):
    """9:16, droitier."""


class LetterKPortraitLeft(Portrait, _LetterKBase):
    """9:16, gaucher."""
