...
#
# paysage droitier  : python -m manim render -qh --disable_caching manim/scripts/cp/en_letter_i.py LetterIRight \
#                       -o eleveai-francais-cp-lettre-i-droitier-paysage --media_dir manim/scripts/cp/media
# paysage gaucher   : python -m manim render -qh --disable_caching manim/scripts/cp/en_letter_i.py LetterILeft \
#                       -o eleveai-francais-cp-lettre-i-gaucher-paysage --media_dir manim/scripts/cp/media
# portrait droitier : python -m manim render -qh --disable_caching -r 1080,1920 manim/scripts/cp/en_letter_i.py LetterIPortraitRight \
#                       -o eleveai-francais-cp-lettre-i-droitier-portrait --media_dir manim/scripts/cp/media
# portrait gaucher  : python -m manim render -qh --disable_caching -r 1080,1920 manim/scripts/cp/en_letter_i.py LetterIPortraitLeft \
...
from lettre_i import igloo_dessine as _igloo  # noqa: E402
...
# ⭐ DESSINS REPRIS DE LA SÉRIE FRANÇAISE : igloo → igloo.
# Même image, même place : c'est ce qui rend la comparaison fr/gb lisible.
MOTS_EN_I = ["igloo", "insect", "ink", "island", "iron"]
...
def image_dessine() -> VGroup:
    """igloo — le dessin de « igloo », repris tel quel."""
    return _igloo()


def igloo_dessine() -> VGroup:
    """An insect: a ladybird — the shell, the seam, the spots, the antennae."""
    # ⭐ « insecte » est un mot vague ; une COCCINELLE ne l'est pas. On dessine
    # l'insecte le plus reconnaissable, pas un insecte générique.
    coque = Circle(radius=0.38, stroke_color=ROUGE_ERREUR, stroke_width=5)
    coque.set_fill(opacity=0)
    couture = Line(np.array([0.00, 0.38, 0]), np.array([0.00, -0.38, 0]),
                   stroke_color=ROUGE_ERREUR, stroke_width=4)
    tete = ArcBetweenPoints(np.array([-0.17, 0.34, 0]), np.array([0.17, 0.34, 0]),
                            angle=-2.0, stroke_color=WHITE, stroke_width=4)
    points = VGroup(
        *[Dot(np.array([x, y, 0]), radius=0.06)
          for x, y in ((-0.18, 0.10), (0.18, 0.10), (-0.15, -0.18),
                       (0.15, -0.18))]
    )
    antennes = VGroup(
        *[Line(np.array([x, 0.40, 0]), np.array([x * 2.2, 0.56, 0]),
               stroke_color=WHITE, stroke_width=3) for x in (-0.09, 0.09)]
    )
    return VGroup(coque, points, couture, tete, antennes)


def iguane_dessine() -> VGroup:
    """Ink: the bottle, its cork, the label, and a drop."""
    flacon = VMobject(stroke_color=BLEU_CALCUL, stroke_width=5)
    flacon.set_fill(opacity=0)
    flacon.start_new_path(np.array([-0.30, -0.42, 0]))
    flacon.add_line_to(np.array([0.30, -0.42, 0]))
    flacon.add_line_to(np.array([0.30, 0.06, 0]))
    flacon.add_line_to(np.array([0.12, 0.22, 0]))
    flacon.add_line_to(np.array([-0.12, 0.22, 0]))
    flacon.add_line_to(np.array([-0.30, 0.06, 0]))
    flacon.add_line_to(np.array([-0.30, -0.42, 0]))
    bouchon = Polygon(
        np.array([-0.11, 0.22, 0]), np.array([0.11, 0.22, 0]),
        np.array([0.11, 0.40, 0]), np.array([-0.11, 0.40, 0]),
        stroke_color=ORANGE_RETENUE, stroke_width=4).set_fill(opacity=0)
    niveau = Line(np.array([-0.30, -0.14, 0]), np.array([0.30, -0.14, 0]),
                  stroke_color=BLEU_CALCUL, stroke_width=3)
    etiquette = Polygon(
        np.array([-0.18, -0.36, 0]), np.array([0.18, -0.36, 0]),
        np.array([0.18, -0.20, 0]), np.array([-0.18, -0.20, 0]),
        stroke_color=WHITE, stroke_width=3).set_fill(opacity=0)
    return VGroup(flacon, niveau, etiquette, bouchon)


def iris_dessine() -> VGroup:
    """An island: the sand, the sea, and ONE palm whose fronds join the trunk."""
    # ⛔ Les palmes partaient d'arcs indépendants : elles FLOTTAIENT au-dessus
    # du tronc. Elles sont maintenant tracées DEPUIS le sommet du tronc, en un
    # seul objet — c'est le point d'attache commun qui fait l'arbre.
    mer = VGroup(
        *[ArcBetweenPoints(np.array([-0.62 + k * 0.30, -0.34, 0]),
                           np.array([-0.44 + k * 0.30, -0.34, 0]),
                           angle=-2.2, stroke_color=BLEU_CALCUL, stroke_width=3)
          for k in range(5)]
    )
    sable = ArcBetweenPoints(np.array([-0.46, -0.28, 0]), np.array([0.46, -0.28, 0]),
                             angle=-0.9, stroke_color=JAUNE_TITRE, stroke_width=5)
    tronc = VMobject(stroke_color=ORANGE_RETENUE, stroke_width=5)
    tronc.set_fill(opacity=0)
    tronc.start_new_path(np.array([-0.06, -0.24, 0]))
    tronc.add_cubic_bezier_curve_to(
        np.array([0.00, 0.00, 0]), np.array([0.06, 0.14, 0]),
        np.array([0.04, 0.30, 0]))
    sommet = np.array([0.04, 0.30, 0])
    palmes = VGroup()
    for dx, dy in ((-0.36, 0.10), (-0.24, -0.14), (0.36, 0.10), (0.24, -0.14)):
        p = VMobject(stroke_color=VERT_OK, stroke_width=4)
        p.set_fill(opacity=0)
        p.start_new_path(sommet)
        p.add_cubic_bezier_curve_to(
            sommet + np.array([dx * 0.4, 0.16, 0]),
            sommet + np.array([dx * 0.8, 0.14, 0]),
            sommet + np.array([dx, dy, 0]))
        palmes.add(p)
    return VGroup(mer, sable, tronc, palmes)

def immeuble_dessine() -> VGroup:
    """An iron: the sole, the body, the handle, and a puff of steam."""
    semelle = VMobject(stroke_color=GREY_B, stroke_width=5)
    semelle.set_fill(opacity=0)
    semelle.start_new_path(np.array([-0.50, -0.34, 0]))
    semelle.add_line_to(np.array([0.34, -0.34, 0]))
    semelle.add_cubic_bezier_curve_to(
        np.array([0.46, -0.34, 0]), np.array([0.50, -0.24, 0]),
        np.array([0.44, -0.18, 0]))
    semelle.add_line_to(np.array([-0.50, -0.18, 0]))
    semelle.add_line_to(np.array([-0.50, -0.34, 0]))
    corps = VMobject(stroke_color=BLEU_CALCUL, stroke_width=5)
    corps.set_fill(opacity=0)
    corps.start_new_path(np.array([-0.46, -0.18, 0]))
    corps.add_line_to(np.array([0.40, -0.18, 0]))
    corps.add_cubic_bezier_curve_to(
        np.array([0.30, 0.10, 0]), np.array([-0.10, 0.14, 0]),
        np.array([-0.46, 0.06, 0]))
    corps.add_line_to(np.array([-0.46, -0.18, 0]))
    poignee = ArcBetweenPoints(np.array([-0.34, 0.08, 0]), np.array([0.22, 0.04, 0]),
                               angle=-1.6, stroke_color=WHITE, stroke_width=5)
    vapeur = VGroup(
        *[ArcBetweenPoints(np.array([x, 0.30, 0]), np.array([x + 0.10, 0.46, 0]),
                           angle=1.6, stroke_color=GREY_B, stroke_width=3)
          for x in (-0.20, 0.06)]
    )
    return VGroup(vapeur, poignee, corps, semelle)


# ─── La voix ──────────────────────────────────────────────────────────────────
# ⚠️ DURÉES MESURÉES par `scripts/generer-voix.ps1`, pas estimées.
VOIX = Path(__file__).resolve().parents[3] / "public" / "sons" / "en-lettre-i"
DUREE = { "00-aujourdhui": 3.62, "01-ecoute": 3.77, "02-regarde": 2.96,
    "03-depart": 10.17, "04-le-point": 5.49, "05-encore": 4.15,
    "06-cherchons": 4.37, "06-i-comme": 1.92, "07-image": 1.54,
    "08-igloo": 1.92, "09-iguane": 1.6, "10-iris": 1.67, "11-immeuble": 1.67,
    "11-pareil": 5.16, "12-relance": 3.46, "13-va-sur": 3.56, "14-tout": 9.71,
    "15-bientot": 2.23,
}
CLIPS_MOTS = ["07-image", "08-igloo", "09-iguane", "10-iris", "11-immeuble"]


class _LetterIBase(Scene):
    ...
    def construct(self):
        ...
        titre_mots = Text("i as in…", font_size=50)
...
class LetterIRight(_LetterIBase):
    """16:9, droitier."""


class LetterILeft(_LetterIBase):
    """16:9, gaucher."""

    gaucher = True


class LetterIPortraitRight(_Portrait, _LetterIBase):
    """9:16, droitier."""


class LetterIPortraitLeft(_Portrait, _LetterIBase):
    """9:16, gaucher."""
