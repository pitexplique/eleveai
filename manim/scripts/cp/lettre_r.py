...
MOTS_EN_R = ["rat", "robot", "roue", "rose", "radis"]
...
def livre_dessine() -> VGroup:
    """Une roue — ⭐ le même dessin que celui des chiffres."""
    jante = Circle(radius=0.40, stroke_color=WHITE, stroke_width=5)
    jante.set_fill(opacity=0)
    moyeu = Circle(radius=0.10, stroke_color=WHITE, stroke_width=4)
    moyeu.set_fill(opacity=0)
    rayons = VGroup(
        *[Line(np.array([0.36 * np.cos(a), 0.36 * np.sin(a), 0]),
               np.array([-0.36 * np.cos(a), -0.36 * np.sin(a), 0]),
               stroke_color=GREY_B, stroke_width=3)
          for a in (0, PI / 3, 2 * PI / 3)]
    )
    return VGroup(jante, rayons, moyeu)
...
def lampe_dessine() -> VGroup:
    """Un radis : la racine rouge, sa pointe blanche, les fanes."""
    bulbe = Circle(radius=0.26, stroke_color=ROUGE_ERREUR, stroke_width=5)
    bulbe.set_fill(opacity=0).move_to(np.array([0, -0.04, 0]))
    pointe = Line(np.array([0, -0.30, 0]), np.array([0.04, -0.56, 0]),
                  stroke_color=WHITE, stroke_width=4)
    fanes = VGroup(
        *[Ellipse(width=0.20, height=0.10, stroke_color=VERT_OK, stroke_width=4)
          .set_fill(opacity=0).rotate(a).move_to(np.array([x, 0.34, 0]))
          for a, x in ((0.7, -0.16), (0.0, 0.02), (-0.7, 0.20))]
    )
    return VGroup(bulbe, pointe, fanes)
...
VOIX = Path(__file__).resolve().parents[3] / "public" / "sons" / "cp-lettre-r"
DUREE = {
    "00-aujourdhui": 3.35, "01-ecoute": 3.10, "02-regarde": 3.10, "03-depart": 18.08,
    "04-encore": 2.92, "05-cherchons": 4.27, "05-r-comme": 1.66, "06-rat": 1.25,
    "07-robot": 1.45, "08-roue": 1.30, "09-rose": 1.30, "10-radis": 1.40,
    "10-pareil": 4.71, "11-relance": 3.08, "12-va-sur": 3.16, "14-bientot": 1.71,
}
CLIPS_MOTS = ["06-rat", "07-robot", "08-roue", "09-rose", "10-radis"]


class _LettreRBase(Scene):
    ...
    def construct(self):
        ...
        titre_mots = Text("r comme…", font_size=50)
        ...
        relance = ecran_relance(self.vertical, "r")
...
class LettreRCp(_LettreRBase):
    """16:9, droitier."""


class LettreRCpGaucher(_LettreRBase):
    """16:9, gaucher."""

    gaucher = True


class LettreRCpPortrait(Portrait, _LettreRBase):
    """9:16, droitier."""


class LettreRCpPortraitGaucher(Portrait, _LettreRBase):
    """9:16, gaucher."""
