...
#
# ⭐⭐ LE SEUL CHIFFRE QUI SE CROISE, ET C'EST LE PLUS EXIGEANT DES DIX.
...
#
# portrait droitier : python -m manim render -qh --disable_caching -r 1080,1920 manim/scripts/cp/chiffre_4.py Chiffre8CpPortrait \
#                       -o eleveai-maths-cp-chiffre-5-droitier-portrait --media_dir manim/scripts/cp/media
# portrait gaucher  : python -m manim render -qh --disable_caching -r 1080,1920 manim/scripts/cp/chiffre_4.py Chiffre8CpPortraitGaucher \
...
# ─── Le chemin du « 8 », d'un seul trait ──────────────────────────────────────
# Forme A, choisie par Frédéric sur trois rendues côte à côte — et c'est celle
# que je recommandais, arrivée par deux chemins.
# ⭐⭐ CE QUI SE JOUE DANS LE CROISEMENT. Le « 8 » est le seul chiffre qui repasse
# sur lui-même. Si le croisement tombe trop haut ou trop bas, on lit un « S »
# fermé ; si les deux boucles sont de même taille, on lit un « ∞ » posé debout.
# ⚠️ D'où le choix : croisement FRANC à mi-hauteur, et deux boucles nettement
# INÉGALES — petite en haut, grande en bas. L'inégalité n'est pas décorative,
# c'est elle qui fait lire « 8 » plutôt qu'« infini ».
# ⭐ Et il se trace d'un seul geste, sans jamais lever : c'est ce qui en fait un
# bon exercice de fin de série — tout ce qu'on a appris s'y retrouve, la boucle
# du 6, la diagonale du 7, la panse du 5.
# ⛔ LA BOUCLE DU HAUT A DÛ ÊTRE ÉLARGIE APRÈS RENDU. Première version : 0,54 de
# large. Sur le papier c'était un « 8 » ; à l'écran, avec un trait de 16, son
# INTÉRIEUR se refermait et le chiffre se lisait « & » ou « S » barré.
# ⚠️ La prévisualisation PIL ne pouvait pas le montrer : le trait y était plus
# fin PAR RAPPORT au chiffre. Une forme ne se valide qu'à l'épaisseur où elle
# sera vue — c'est la même leçon que les dessins qui deviennent des taches à
# neuf exemplaires.
# ⭐ Le croisement descend aussi un peu : il donne de la place au haut sans
# écraser le bas, et les deux boucles restent franchement inégales — c'est
# l'inégalité qui fait lire « 8 » plutôt qu'« infini ».
DEPART = np.array([0.32, 1.90, 0])
COURBES = [
    ((0.02, 2.06), (-0.36, 1.84), (-0.36, 1.50)),    # la boucle du haut, ÉLARGIE
    ((-0.36, 1.20), (0.14, 1.06), (0.28, 0.78)),     # on descend en biais
    ((0.42, 0.48), (0.22, 0.02), (-0.06, 0.02)),     # le grand rond du bas
    ((-0.38, 0.02), (-0.50, 0.42), (-0.22, 0.68)),   # on remonte à gauche
    ((0.04, 0.92), (0.34, 1.12), (0.34, 1.44)),      # la diagonale qui CROISE
    ((0.34, 1.78), (0.06, 2.00), (0.32, 1.90)),      # et on ferme en haut
]


def chemin_8(stroke_width: float = 10, color: str = WHITE) -> VGroup:
...
VOIX = Path(__file__).resolve().parents[3] / "public" / "sons" / "cp-chiffre-8"
DUREE = {
    "00-aujourdhui": 4.19, "01-ecoute": 2.50, "02-regarde": 4.02, "03-depart": 20.60,
...
class _Chiffre8Base(Scene):
    ...
    def construct(self):
        ...
        garde, garde_main = page_de_garde(
            ...
            self, CHIFFRE, chemin_8(stroke_width=12), MascotteMargouillat(),
        ...
        traits = chemin_8(stroke_width=14 if not self.vertical else 16)
        VGroup(lignes, traits).move_to(ORIGIN).shift(DOWN * 0.3)

        modele = chemin_8(stroke_width=14 if not self.vertical else 16, color=GREY_D)
        ...
        for duree in (2.0, 1.2):
            if refait is not None:
                ...
                self.remove(*refait)
            refait = chemin_8(stroke_width=14 if not self.vertical else 16)
...
class Chiffre8Cp(_Chiffre8Base):
    """16:9, droitier — conservée, mais on ne la rend plus."""


class Chiffre8CpGaucher(_Chiffre8Base):
    gaucher = True


class Chiffre8CpPortrait(Portrait, _Chiffre8Base):
    """9:16, droitier — LE format qui est vu."""


class Chiffre8CpPortraitGaucher(Portrait, _Chiffre8Base):
    gaucher = True
