...
MOTS_EN_P = ["pomme", "poule", "porte", "pain", "panier"]
...
def lune_dessine() -> VGroup:
    """Une pomme : le fruit, la queue, la feuille."""
    fruit = Circle(radius=0.36, stroke_color=ROUGE_ERREUR, stroke_width=5)
    fruit.set_fill(opacity=0)
    queue = Line(np.array([0.02, 0.34, 0]), np.array([0.06, 0.58, 0]),
                 stroke_color=ORANGE_RETENUE, stroke_width=4)
    feuille = Ellipse(width=0.24, height=0.12, stroke_color=VERT_OK, stroke_width=4)
    feuille.set_fill(opacity=0).rotate(0.5).move_to(np.array([0.20, 0.52, 0]))
    return VGroup(fruit, queue, feuille)


def lit_dessine() -> VGroup:
    """Une poule : le corps, la tête, la crête, le bec."""
    corps = Ellipse(width=0.72, height=0.52, stroke_color=WHITE, stroke_width=5)
    corps.set_fill(opacity=0).move_to(np.array([-0.08, -0.14, 0]))
    tete = Circle(radius=0.19, stroke_color=WHITE, stroke_width=5)
    tete.set_fill(opacity=0).move_to(np.array([0.30, 0.24, 0]))
    crete = Polygon(np.array([0.22, 0.40, 0]), np.array([0.30, 0.54, 0]),
                    np.array([0.38, 0.40, 0]),
                    stroke_color=ROUGE_ERREUR, stroke_width=4).set_fill(opacity=0)
    bec = Polygon(np.array([0.46, 0.26, 0]), np.array([0.64, 0.20, 0]),
                  np.array([0.46, 0.14, 0]),
                  stroke_color=ORANGE_RETENUE, stroke_width=4).set_fill(opacity=0)
    oeil = Dot(np.array([0.36, 0.28, 0]), radius=0.035)
    pattes = VGroup(
        *[Line(np.array([x, -0.40, 0]), np.array([x, -0.56, 0]),
               stroke_color=ORANGE_RETENUE, stroke_width=4) for x in (-0.20, 0.04)]
    )
    return VGroup(corps, tete, crete, bec, oeil, pattes)


def livre_dessine() -> VGroup:
    """Une porte : le battant, le cadre, la poignée."""
    cadre = Polygon(
        np.array([-0.32, -0.48, 0]), np.array([0.32, -0.48, 0]),
        np.array([0.32, 0.52, 0]), np.array([-0.32, 0.52, 0]),
        stroke_color=ORANGE_RETENUE, stroke_width=5,
    ).set_fill(opacity=0)
    panneau = Polygon(
        np.array([-0.20, -0.32, 0]), np.array([0.20, -0.32, 0]),
        np.array([0.20, 0.34, 0]), np.array([-0.20, 0.34, 0]),
        stroke_color=ORANGE_RETENUE, stroke_width=3,
    ).set_fill(opacity=0)
    poignee = Dot(np.array([0.22, 0.02, 0]), radius=0.05, color=JAUNE_TITRE)
    return VGroup(cadre, panneau, poignee)


def lapin_dessine() -> VGroup:
    """Un pain : la miche allongée et ses entailles."""
    miche = Ellipse(width=1.00, height=0.44, stroke_color=ORANGE_RETENUE,
                    stroke_width=5).set_fill(opacity=0)
    entailles = VGroup(
        *[Line(np.array([-0.26 + k * 0.26, 0.10, 0]),
               np.array([-0.14 + k * 0.26, 0.20, 0]),
               stroke_color=ORANGE_RETENUE, stroke_width=3) for k in range(3)]
    )
    return VGroup(miche, entailles)


def lampe_dessine() -> VGroup:
    """Un panier : la corbeille tressée et son anse."""
    corbeille = Polygon(
        np.array([-0.44, 0.14, 0]), np.array([0.44, 0.14, 0]),
        np.array([0.32, -0.34, 0]), np.array([-0.32, -0.34, 0]),
        stroke_color=ORANGE_RETENUE, stroke_width=5,
    ).set_fill(opacity=0)
    tresse = VGroup(
        *[Line(np.array([-0.40 + k * 0.20, 0.14, 0]),
               np.array([-0.30 + k * 0.16, -0.34, 0]),
               stroke_color=ORANGE_RETENUE, stroke_width=3) for k in range(5)]
    )
    anse = ArcBetweenPoints(np.array([-0.36, 0.14, 0]), np.array([0.36, 0.14, 0]),
                            angle=-2.0, stroke_color=ORANGE_RETENUE, stroke_width=4)
    return VGroup(corbeille, tresse, anse)
...
VOIX = Path(__file__).resolve().parents[3] / "public" / "sons" / "cp-lettre-p"
DUREE = {
    "00-aujourdhui": 3.35, "01-ecoute": 3.10, "02-regarde": 3.10, "03-depart": 22.60,
    "04-encore": 2.92, "05-cherchons": 4.27, "05-p-comme": 1.66, "06-pomme": 1.45,
    "07-poule": 1.45, "08-porte": 1.45, "09-pain": 1.30, "10-panier": 1.55,
    "10-pareil": 4.71, "11-relance": 3.08, "12-va-sur": 3.16, "14-bientot": 1.71,
}
CLIPS_MOTS = ["06-pomme", "07-poule", "08-porte", "09-pain", "10-panier"]


class _LettrePBase(Scene):
    ...
    def construct(self):
        ...
        titre_mots = Text("p comme…", font_size=50)
        ...
        relance = ecran_relance(self.vertical, "p")
...
class LettrePCp(_LettrePBase):
    """16:9, droitier."""


class LettrePCpGaucher(_LettrePBase):
    """16:9, gaucher."""

    gaucher = True


class LettrePCpPortrait(Portrait, _LettrePBase):
    """9:16, droitier."""


class LettrePCpPortraitGaucher(Portrait, _LettrePBase):
    """9:16, gaucher."""
