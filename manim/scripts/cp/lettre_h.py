# Écriture de la lettre « h » en cursive — CP
#
# ⭐⭐ LA SEULE LETTRE QUI NE SE PRONONCE PAS, et il a fallu réécrire le script
# pour ça. « Écoute bien : h » n'a aucun sens — il n'y a rien à entendre. La
# voix dit donc : « le h, c'est une lettre spéciale : on l'écrit, mais on ne
# l'entend pas ! », et la conclusion des cinq mots devient « le h est écrit au
# début, mais on ne l'entend jamais ».
# ⚠️ Ce n'est pas un contournement, c'est LA leçon du « h » au CP. Les treize
# lettres précédentes associaient une lettre à un son ; celle-ci enseigne
# qu'une lettre peut n'en avoir aucun.
#
...
# ⭐ LE « h » = LA BOUCLE DU « l » + LE PONT DU « n ».
# Assemblé à partir de deux gestes déjà validés, et juste du premier coup — la
# démonstration que la grammaire des gestes paie, comme pour le « m ».
#   1. le creux qui monte, la voûte, la boucle à gauche (du « l ») ;
#   2. la descente droite jusqu'à la ligne ;
#   3. la remontée SUR LE MÊME TRAIT (du « n ») ;
#   4. le pont ;
#   5. le creux final — le même que l, t, n, m, p, r.
H = 1.00
DEPART = np.array([-0.90, 0.08, 0])
COURBES = [
    ((-0.60, 0.10), (-0.20, 0.55), (0.10, 1.30)),   # 1. le creux qui monte
    ((0.24, 1.90), (0.34, 2.50), (0.34, 2.95)),     # …vers le sommet
    ((0.34, 3.30), (-0.10, 3.32), (-0.18, 2.80)),   # la voûte, qui bascule à GAUCHE
    ((-0.10, 2.00), (0.00, 1.00), (0.04, 0.05)),    # 2. la descente, DROITE
    ((0.04, 0.20), (0.04, 0.38), (0.05, 0.54)),     # 3. la remontée, MÊME TRAIT
    ((0.11, 0.88), (0.34, H + 0.04), (0.50, H - 0.02)),  # 4. le pont
    ((0.58, 0.72), (0.59, 0.34), (0.59, 0.08)),     # la descente
    ((0.61, -0.02), (0.76, -0.03), (0.90, 0.10)),   # 5. le creux, sur la ligne
    ((1.00, 0.20), (1.08, 0.28), (1.18, 0.34)),     # la sortie, qui s'effile
]
...
# ⚠️ Cinq mots où le « h » est ÉCRIT en initiale — mais jamais entendu. C'est
# tout le propos de cette vidéo.
MOTS_EN_H = ["hibou", "hache", "haricot", "hôtel", "hamac"]


def chemin_h(stroke_width: float = 10, color: str = WHITE) -> VMobject:
    return chemin_bezier(DEPART, COURBES, stroke_width, color)


# ─── Les cinq dessins ─────────────────────────────────────────────────────────
def lune_dessine() -> VGroup:
    """Un hibou : le corps, deux grands yeux, les aigrettes, le bec."""
    corps = Ellipse(width=0.62, height=0.76, stroke_color=ORANGE_RETENUE,
                    stroke_width=5).set_fill(opacity=0)
    yeux = VGroup(
        *[Circle(radius=0.11, stroke_color=JAUNE_TITRE, stroke_width=4)
          .set_fill(opacity=0).move_to(np.array([x, 0.16, 0])) for x in (-0.13, 0.13)]
    )
    pupilles = VGroup(*[Dot(np.array([x, 0.16, 0]), radius=0.04)
                        for x in (-0.13, 0.13)])
    bec = Polygon(np.array([-0.05, 0.02, 0]), np.array([0.05, 0.02, 0]),
                  np.array([0.00, -0.10, 0]),
                  stroke_color=JAUNE_TITRE, stroke_width=4).set_fill(opacity=0)
    aigrettes = VGroup(
        *[Polygon(np.array([x - 0.06, 0.34, 0]), np.array([x, 0.52, 0]),
                  np.array([x + 0.06, 0.34, 0]),
                  stroke_color=ORANGE_RETENUE, stroke_width=4).set_fill(opacity=0)
          for x in (-0.16, 0.16)]
    )
    return VGroup(aigrettes, corps, yeux, pupilles, bec)


def lit_dessine() -> VGroup:
    """Une hache : le manche et le fer."""
    manche = Line(np.array([-0.22, -0.44, 0]), np.array([0.10, 0.34, 0]),
                  stroke_color=ORANGE_RETENUE, stroke_width=6)
    fer = Polygon(
        np.array([0.04, 0.22, 0]), np.array([0.42, 0.30, 0]),
        np.array([0.34, 0.56, 0]), np.array([-0.02, 0.42, 0]),
        stroke_color=GREY_B, stroke_width=5).set_fill(opacity=0)
    return VGroup(manche, fer)


def livre_dessine() -> VGroup:
    """Un haricot : la gousse et trois graines."""
    gousse = VMobject(stroke_color=VERT_OK, stroke_width=5)
    gousse.set_fill(opacity=0)
    gousse.start_new_path(np.array([-0.50, -0.16, 0]))
    gousse.add_cubic_bezier_curve_to(
        np.array([-0.22, 0.34, 0]), np.array([0.22, 0.34, 0]),
        np.array([0.50, -0.16, 0]))
    gousse.add_cubic_bezier_curve_to(
        np.array([0.22, 0.06, 0]), np.array([-0.22, 0.06, 0]),
        np.array([-0.50, -0.16, 0]))
    graines = VGroup(
        *[Circle(radius=0.06, stroke_color=VERT_OK, stroke_width=3)
          .set_fill(opacity=0).move_to(np.array([x, 0.02, 0]))
          for x in (-0.20, 0.00, 0.20)]
    )
    return VGroup(gousse, graines)


def lapin_dessine() -> VGroup:
    """Un hôtel : l'immeuble, ses fenêtres, sa porte, son enseigne."""
    murs = Polygon(
        np.array([-0.36, -0.44, 0]), np.array([0.36, -0.44, 0]),
        np.array([0.36, 0.40, 0]), np.array([-0.36, 0.40, 0]),
        stroke_color=WHITE, stroke_width=5).set_fill(opacity=0)
    fenetres = VGroup(
        *[Polygon(np.array([x - 0.08, y - 0.07, 0]), np.array([x + 0.08, y - 0.07, 0]),
                  np.array([x + 0.08, y + 0.07, 0]), np.array([x - 0.08, y + 0.07, 0]),
                  stroke_color=BLEU_CALCUL, stroke_width=3).set_fill(opacity=0)
          for y in (0.20, -0.02) for x in (-0.18, 0.18)]
    )
    porte = Polygon(
        np.array([-0.09, -0.44, 0]), np.array([0.09, -0.44, 0]),
        np.array([0.09, -0.18, 0]), np.array([-0.09, -0.18, 0]),
        stroke_color=ORANGE_RETENUE, stroke_width=4).set_fill(opacity=0)
    enseigne = Line(np.array([-0.24, 0.52, 0]), np.array([0.24, 0.52, 0]),
                    stroke_color=JAUNE_TITRE, stroke_width=5)
    return VGroup(murs, fenetres, porte, enseigne)


def lampe_dessine() -> VGroup:
    """Un hamac : la toile suspendue entre deux troncs."""
    toile = VMobject(stroke_color=BLEU_CALCUL, stroke_width=5)
    toile.set_fill(opacity=0)
    toile.start_new_path(np.array([-0.44, 0.16, 0]))
    toile.add_cubic_bezier_curve_to(
        np.array([-0.24, -0.40, 0]), np.array([0.24, -0.40, 0]),
        np.array([0.44, 0.16, 0]))
    mailles = VGroup(
        *[Line(np.array([x, 0.10, 0]), np.array([x * 0.9, -0.24, 0]),
               stroke_color=BLEU_CALCUL, stroke_width=2)
          for x in (-0.26, -0.09, 0.09, 0.26)]
    )
    troncs = VGroup(
        *[Line(np.array([x, 0.40, 0]), np.array([x, -0.44, 0]),
               stroke_color=ORANGE_RETENUE, stroke_width=5) for x in (-0.56, 0.56)]
    )
    return VGroup(troncs, toile, mailles)
...
VOIX = Path(__file__).resolve().parents[3] / "public" / "sons" / "cp-lettre-h"
DUREE = {
    "00-aujourdhui": 3.35, "01-ecoute": 6.02, "02-regarde": 3.10, "03-depart": 20.99,
    "04-encore": 2.92, "05-cherchons": 4.27, "05-h-comme": 1.66, "06-hibou": 1.40,
    "07-hache": 1.35, "08-haricot": 1.55, "09-hotel": 1.40, "10-hamac": 1.45,
    "10-pareil": 6.05, "11-relance": 3.08, "12-va-sur": 3.16, "14-bientot": 1.71,
}
CLIPS_MOTS = ["06-hibou", "07-hache", "08-haricot", "09-hotel", "10-hamac"]


class _LettreHBase(Scene):
    ...
    def construct(self):
        son = Text("h", font_size=150, color=JAUNE_TITRE)
        ...
        garde, garde_main = page_de_garde(
            self, "h", chemin_h(stroke_width=12), MascotteMargouillat()
        )
        ...
        lignes = reglure(3)
        lettre = chemin_h(stroke_width=14 if not self.vertical else 16)
        ...
        modele = chemin_h(stroke_width=14 if not self.vertical else 16, color=GREY_D)
        modele.match_points(lettre)
        imprime = Text(
            "h", font_size=110 if not self.vertical else 90, color=JAUNE_TITRE
        ).move_to(modele)
        ...
        for duree in (2.0, 1.2):
            self.remove(trace)
            trace = chemin_h(stroke_width=14 if not self.vertical else 16)
        ...
        titre_mots = Text("h comme…", font_size=50)
        ...
        lignes_mots = VGroup()
        for mot, dessin in zip(MOTS_EN_H, dessins):
        ...
        relance = ecran_relance(self.vertical, "h")
...
class LettreHCp(_LettreHBase):
    """16:9, droitier."""


class LettreHCpGaucher(_LettreHBase):
    """16:9, gaucher."""

    gaucher = True


class LettreHCpPortrait(Portrait, _LettreHBase):
    """9:16, droitier."""


class LettreHCpPortraitGaucher(Portrait, _LettreHBase):
    """9:16, gaucher."""
