...
MOTS_EN_C = ["cadeau", "carotte", "canard", "clé", "cactus"]
...
def lune_dessine() -> VGroup:
    """Un cadeau : la boite, le ruban, le nœud."""
    boite = Polygon(
        np.array([-0.38, -0.34, 0]), np.array([0.38, -0.34, 0]),
        np.array([0.38, 0.20, 0]), np.array([-0.38, 0.20, 0]),
        stroke_color=ROUGE_ERREUR, stroke_width=5).set_fill(opacity=0)
    ruban_h = Line(np.array([0.00, -0.34, 0]), np.array([0.00, 0.20, 0]),
                   stroke_color=JAUNE_TITRE, stroke_width=4)
    ruban_v = Line(np.array([-0.38, -0.06, 0]), np.array([0.38, -0.06, 0]),
                   stroke_color=JAUNE_TITRE, stroke_width=4)
    noeud = VGroup(
        *[Ellipse(width=0.20, height=0.14, stroke_color=JAUNE_TITRE, stroke_width=4)
          .set_fill(opacity=0).move_to(np.array([x, 0.32, 0])) for x in (-0.11, 0.11)]
    )
    return VGroup(boite, ruban_h, ruban_v, noeud)
...
def livre_dessine() -> VGroup:
    """Un canard : le corps, la tête, le bec, l'eau."""
    corps = Ellipse(width=0.72, height=0.38, stroke_color=JAUNE_TITRE, stroke_width=5)
    corps.set_fill(opacity=0).move_to(np.array([-0.08, -0.14, 0]))
    tete = Circle(radius=0.17, stroke_color=JAUNE_TITRE, stroke_width=5)
    tete.set_fill(opacity=0).move_to(np.array([0.26, 0.22, 0]))
    cou = Line(np.array([0.14, -0.04, 0]), np.array([0.24, 0.10, 0]),
               stroke_color=JAUNE_TITRE, stroke_width=5)
    bec = Polygon(np.array([0.40, 0.26, 0]), np.array([0.62, 0.18, 0]),
                  np.array([0.40, 0.12, 0]),
                  stroke_color=ORANGE_RETENUE, stroke_width=4).set_fill(opacity=0)
    oeil = Dot(np.array([0.30, 0.28, 0]), radius=0.03)
    eau = Line(np.array([-0.56, -0.36, 0]), np.array([0.48, -0.36, 0]),
               stroke_color=BLEU_CALCUL, stroke_width=3)
    return VGroup(eau, corps, cou, tete, bec, oeil)


def lapin_dessine() -> VGroup:
    """Une clé : l'anneau, la tige, les dents."""
    anneau = Circle(radius=0.18, stroke_color=JAUNE_TITRE, stroke_width=5)
    anneau.set_fill(opacity=0).move_to(np.array([-0.34, 0.00, 0]))
    tige = Line(np.array([-0.16, 0.00, 0]), np.array([0.48, 0.00, 0]),
                stroke_color=JAUNE_TITRE, stroke_width=5)
    dents = VGroup(
        *[Line(np.array([x, 0.00, 0]), np.array([x, -0.18, 0]),
               stroke_color=JAUNE_TITRE, stroke_width=4) for x in (0.28, 0.44)]
    )
    return VGroup(anneau, tige, dents)
...
VOIX = Path(__file__).resolve().parents[3] / "public" / "sons" / "cp-lettre-c"
DUREE = {
    "00-aujourdhui": 3.35, "01-ecoute": 3.10, "02-regarde": 3.10, "03-depart": 17.60,
    "04-encore": 2.92, "05-cherchons": 4.27, "05-c-comme": 1.66, "06-cadeau": 1.45,
    "07-carotte": 1.50, "08-canard": 1.45, "09-cle": 1.15, "10-cactus": 1.45,
    "10-pareil": 4.71, "11-relance": 3.08, "12-va-sur": 3.16, "14-bientot": 1.71,
}
CLIPS_MOTS = ["06-cadeau", "07-carotte", "08-canard", "09-cle", "10-cactus"]


class _LettreCBase(Scene):
    ...
    def construct(self):
        ...
        titre_mots = Text("c comme…", font_size=50)
        ...
        relance = ecran_relance(self.vertical, "c")
...
class LettreCCp(_LettreCBase):
    """16:9, droitier."""


class LettreCCpGaucher(_LettreCBase):
    """16:9, gaucher."""

    gaucher = True


class LettreCCpPortrait(Portrait, _LettreCBase):
    """9:16, droitier."""


class LettreCCpPortraitGaucher(Portrait, _LettreCBase):
    """9:16, gaucher."""
