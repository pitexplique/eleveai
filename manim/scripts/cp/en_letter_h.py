...
#
# ⛔⛔ EN ANGLAIS, LE « h » SE PRONONCE — hat, house, hand. La leçon du « on ne
# l'entend pas », qui est le cœur de la version française, serait ici une
# ERREUR : on enseignerait aux enfants anglophones que leur « h » est muet.
# ⚠️ C'est le premier cas où une lettre change de LEÇON en changeant de langue,
# et pas seulement de vocabulaire. Traduire, ce n'est décidément pas traduire.
#
# ⭐ Quatre des cinq mots réutilisent des dessins existants : la maison et la
# montagne du « m », la main du chiffre « 5 ».
#
...
MOTS_EN_H = ["hat", "house", "hand", "heart", "hill"]
...
def lune_dessine() -> VGroup:
    """A hat: the crown and the brim."""
    calotte = VMobject(stroke_color=BLEU_CALCUL, stroke_width=5)
    calotte.set_fill(opacity=0)
    calotte.start_new_path(np.array([-0.28, 0.04, 0]))
    calotte.add_cubic_bezier_curve_to(
        np.array([-0.26, 0.48, 0]), np.array([0.26, 0.48, 0]),
        np.array([0.28, 0.04, 0]))
    bord = Ellipse(width=0.96, height=0.16, stroke_color=BLEU_CALCUL,
                   stroke_width=5).set_fill(opacity=0).move_to(np.array([0, 0.02, 0]))
    ruban = Line(np.array([-0.27, 0.12, 0]), np.array([0.27, 0.12, 0]),
                 stroke_color=ROUGE_ERREUR, stroke_width=4)
    return VGroup(bord, calotte, ruban)


def lit_dessine() -> VGroup:
    """A house — the same drawing as the French « maison » of the « m »."""
    murs = Polygon(
    ...
    return VGroup(murs, toit, porte)


def livre_dessine() -> VGroup:
    """A hand — the same drawing as the body referent of the digit « 5 »."""
    return main_ouverte(5).scale(0.72)


def lapin_dessine() -> VGroup:
    """A heart: two lobes and a point."""
    coeur = VMobject(stroke_color=ROUGE_ERREUR, stroke_width=5)
    coeur.set_fill(opacity=0)
    coeur.start_new_path(np.array([0.00, -0.40, 0]))
    coeur.add_cubic_bezier_curve_to(
        np.array([-0.50, 0.02, 0]), np.array([-0.44, 0.50, 0]),
        np.array([0.00, 0.24, 0]))
    coeur.add_cubic_bezier_curve_to(
        np.array([0.44, 0.50, 0]), np.array([0.50, 0.02, 0]),
        np.array([0.00, -0.40, 0]))
    return VGroup(coeur)


def lampe_dessine() -> VGroup:
    """A hill — the same drawing as the French « montagne » of the « m »."""
    massif = Polygon(
        np.array([-0.62, -0.34, 0]), np.array([-0.16, 0.42, 0]),
        np.array([0.06, 0.10, 0]), np.array([0.30, 0.50, 0]),
        np.array([0.62, -0.34, 0]),
        stroke_color=VERT_OK, stroke_width=5).set_fill(opacity=0)
    soleil = Circle(radius=0.12, stroke_color=JAUNE_TITRE, stroke_width=4)
    soleil.set_fill(opacity=0).move_to(np.array([0.46, 0.42, 0]))
    return VGroup(massif, soleil)
...
VOIX = Path(__file__).resolve().parents[3] / "public" / "sons" / "en-lettre-h"
DUREE = {
    "00-aujourdhui": 3.60, "01-ecoute": 3.70, "02-regarde": 3.00, "03-depart": 27.66,
    "04-encore": 4.15, "05-cherchons": 4.50, "05-h-comme": 1.80, "06-hibou": 1.20,
    "07-hache": 1.35, "08-haricot": 1.30, "09-hotel": 1.35, "10-hamac": 1.25,
    "10-pareil": 5.16, "11-relance": 3.60, "12-va-sur": 3.56, "14-bientot": 2.23,
}
CLIPS_MOTS = ["06-hibou", "07-hache", "08-haricot", "09-hotel", "10-hamac"]


class _LetterHBase(Scene):
    ...
    def construct(self):
        ...
        titre_mots = Text("h as in…", font_size=50)
...
class LetterHRight(_LetterHBase):
    """16:9, droitier."""


class LetterHLeft(_LetterHBase):
    """16:9, gaucher."""

    gaucher = True


class LetterHPortraitRight(Portrait, _LetterHBase):
    """9:16, droitier."""


class LetterHPortraitLeft(Portrait, _LetterHBase):
    """9:16, gaucher."""
