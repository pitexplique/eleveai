# Writing the cursive letter « f » — CP (English)
#
...
# ⭐ « forest » et « fairy » GARDENT LEUR DESSIN FRANÇAIS (forêt, fée) : même
# image, même ordre, la comparaison fr/gb reste lisible.
# « fourmi », « feuille » et « fusée » n'ont pas d'équivalent en f — leurs mots
# anglais commencent par a, l et r. Trois COUPLES neufs : fish, feather, fox.
# ⛔ TRADUIRE UNE LETTRE, C'EST TRADUIRE CINQ COUPLES MOT-IMAGE, PAS CINQ MOTS.
# Le 06/09 j'ai failli laisser « tent » sur le dessin d'un tapis.
# ⭐ CINQ SUR CINQ SONT DU VIVANT OU DU CIEL — l'anglais fait mieux que le
# français ici, où la fusée tenait la place.
MOTS_EN_F = ["fish", "feather", "forest", "fox", "fairy"]
...
def lune_dessine() -> VGroup:
    """A fish: the body, the tail, an eye and a fin."""
    corps = VMobject(stroke_color=BLEU_CALCUL, stroke_width=5)
    corps.set_fill(opacity=0)
    corps.start_new_path(np.array([-0.44, 0.00, 0]))
    corps.add_cubic_bezier_curve_to(
        np.array([-0.24, 0.32, 0]), np.array([0.16, 0.30, 0]),
        np.array([0.34, 0.00, 0]))
    corps.add_cubic_bezier_curve_to(
        np.array([0.16, -0.30, 0]), np.array([-0.24, -0.32, 0]),
        np.array([-0.44, 0.00, 0]))
    queue = Polygon(np.array([0.34, 0.00, 0]), np.array([0.62, 0.24, 0]),
                    np.array([0.62, -0.24, 0]),
                    stroke_color=BLEU_CALCUL, stroke_width=4).set_fill(opacity=0)
    nageoire = Polygon(np.array([-0.12, 0.22, 0]), np.array([0.02, 0.44, 0]),
                       np.array([0.10, 0.20, 0]),
                       stroke_color=BLEU_CALCUL, stroke_width=3).set_fill(opacity=0)
    oeil = Dot(np.array([-0.28, 0.06, 0]), radius=0.035)
    bulle = Circle(radius=0.05, stroke_color=BLEU_CALCUL, stroke_width=2)
    bulle.set_fill(opacity=0).move_to(np.array([-0.58, 0.24, 0]))
    return VGroup(corps, queue, nageoire, oeil, bulle)


def lit_dessine() -> VGroup:
    """A feather: the shaft, and short barbs combed on BOTH sides."""
    # ⛔ PREMIÈRE VERSION : les barbes en zigzag, toutes du même côté — ça
    # lisait comme un éclair posé sur une feuille. Une plume se reconnait à ses
    # barbes PEIGNÉES, courtes, régulières, et des DEUX côtés du tuyau.
    tige = Line(np.array([-0.34, -0.46, 0]), np.array([0.26, 0.48, 0]),
                stroke_color=JAUNE_TITRE, stroke_width=4)
    contour = VMobject(stroke_color=JAUNE_TITRE, stroke_width=4)
    contour.set_fill(opacity=0)
    contour.start_new_path(np.array([0.26, 0.48, 0]))
    contour.add_cubic_bezier_curve_to(
        np.array([-0.16, 0.42, 0]), np.array([-0.36, 0.06, 0]),
        np.array([-0.24, -0.30, 0]))
    contour.add_cubic_bezier_curve_to(
        np.array([0.12, -0.10, 0]), np.array([0.34, 0.16, 0]),
        np.array([0.26, 0.48, 0]))
    barbes = VGroup()
    for k in range(5):
        t = 0.16 + k * 0.15
        x = -0.34 + t * 0.60
        y = -0.46 + t * 0.94
        barbes.add(
            Line(np.array([x, y, 0]), np.array([x - 0.13, y + 0.04, 0]),
                 stroke_color=JAUNE_TITRE, stroke_width=2),
            Line(np.array([x, y, 0]), np.array([x + 0.11, y - 0.05, 0]),
                 stroke_color=JAUNE_TITRE, stroke_width=2),
        )
...
def lapin_dessine() -> VGroup:
    """A fox: the pointed face alone — pointed muzzle, two big ears."""
    # ⛔ PREMIÈRE VERSION : une queue touffue à gauche de la tête. Détachée du
    # corps (qu'il n'y avait pas), elle lisait comme une tache posée à côté.
    # Un museau pointu et deux grandes oreilles suffisent à dire « renard » ;
    # ajouter un membre sans le corps qui le porte n'ajoute rien, ça brouille.
    tete = Polygon(np.array([-0.30, 0.20, 0]), np.array([0.30, 0.20, 0]),
                   np.array([0.00, -0.36, 0]),
                   stroke_color=ORANGE_RETENUE, stroke_width=5).set_fill(opacity=0)
    oreilles = VGroup(
        *[Polygon(np.array([x - 0.13, 0.18, 0]), np.array([x, 0.58, 0]),
                  np.array([x + 0.13, 0.18, 0]),
                  stroke_color=ORANGE_RETENUE, stroke_width=4).set_fill(opacity=0)
          for x in (-0.17, 0.17)]
    )
    yeux = VGroup(*[Dot(np.array([x, 0.04, 0]), radius=0.04) for x in (-0.11, 0.11)])
    truffe = Dot(np.array([0.00, -0.30, 0]), radius=0.05)
    joues = VGroup(
        *[Line(np.array([s * 0.18, -0.10, 0]), np.array([s * 0.40, -0.06, 0]),
               stroke_color=ORANGE_RETENUE, stroke_width=2) for s in (-1, 1)]
    )
    return VGroup(oreilles, tete, yeux, truffe, joues)
...
VOIX = Path(__file__).resolve().parents[3] / "public" / "sons" / "en-lettre-f"
DUREE = { "00-aujourdhui": 3.85, "01-ecoute": 4.0, "02-regarde": 3.19,
    "03-depart": 31.34, "04-encore": 4.15, "05-cherchons": 4.59,
    "05-f-comme": 1.9, "06-fourmi": 1.7, "07-feuille": 1.68, "08-foret": 1.95,
    "09-fusee": 1.85, "10-fee": 1.86, "10-pareil": 5.16, "11-relance": 3.68,
    "12-va-sur": 3.56, "14-bientot": 2.23,
}
CLIPS_MOTS = ["06-fourmi", "07-feuille", "08-foret", "09-fusee", "10-fee"]


class _LetterFBase(Scene):
    ...
    def construct(self):
        ...
        titre_mots = Text("f as in…", font_size=50)
...
class LetterFRight(_LetterFBase):
    """16:9, droitier."""


class LetterFLeft(_LetterFBase):
    """16:9, gaucher."""

    gaucher = True


class LetterFPortraitRight(Portrait, _LetterFBase):
    """9:16, droitier."""


class LetterFPortraitLeft(Portrait, _LetterFBase):
    """9:16, gaucher."""
