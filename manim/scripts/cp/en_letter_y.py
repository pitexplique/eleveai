...
MOTS_EN_Y = [("yo-yo", 0), ("yacht", 0), ("yarn", 0), ("yolk", 0), ("yoghurt", 0)]
...
def stylo_dessine() -> VGroup:
    """A yo-yo: the two discs seen edge-on, and the string with its loop."""
    disque = Circle(radius=0.32, stroke_color=ROUGE_ERREUR, stroke_width=5)
    disque.set_fill(opacity=0).move_to(np.array([0.00, -0.16, 0]))
    axe = Ellipse(width=0.10, height=0.14, stroke_color=ROUGE_ERREUR,
                  stroke_width=3)
    axe.set_fill(opacity=0).move_to(np.array([0.00, -0.16, 0]))
    tranche = ArcBetweenPoints(np.array([-0.30, -0.05, 0]), np.array([0.30, -0.05, 0]),
                               angle=-0.5, stroke_color=ROUGE_ERREUR,
                               stroke_width=3)
    ficelle = Line(np.array([0.00, -0.16, 0]), np.array([0.00, 0.46, 0]),
                   stroke_color=WHITE, stroke_width=3)
    boucle = Circle(radius=0.08, stroke_color=WHITE, stroke_width=3)
    boucle.set_fill(opacity=0).move_to(np.array([0.00, 0.54, 0]))
    return VGroup(ficelle, boucle, disque, tranche, axe)


def pyjama_dessine() -> VGroup:
    """A yacht: the hull, the mast, two sails, and the water."""
    coque = VMobject(stroke_color=WHITE, stroke_width=5)
    coque.set_fill(opacity=0)
    coque.start_new_path(np.array([-0.52, -0.20, 0]))
    coque.add_line_to(np.array([0.52, -0.20, 0]))
    coque.add_cubic_bezier_curve_to(
        np.array([0.34, -0.44, 0]), np.array([-0.34, -0.44, 0]),
        np.array([-0.52, -0.20, 0]))
    mat = Line(np.array([0.00, -0.20, 0]), np.array([0.00, 0.58, 0]),
               stroke_color=ORANGE_RETENUE, stroke_width=4)
    grande = Polygon(np.array([0.04, 0.54, 0]), np.array([0.04, -0.16, 0]),
                     np.array([0.42, -0.16, 0]),
                     stroke_color=ROUGE_ERREUR, stroke_width=4).set_fill(opacity=0)
    foc = Polygon(np.array([-0.04, 0.50, 0]), np.array([-0.04, -0.16, 0]),
                  np.array([-0.34, -0.16, 0]),
                  stroke_color=BLEU_CALCUL, stroke_width=4).set_fill(opacity=0)
    vagues = VGroup(
        *[ArcBetweenPoints(np.array([-0.60 + k * 0.30, -0.42, 0]),
                           np.array([-0.42 + k * 0.30, -0.42, 0]),
                           angle=-2.2, stroke_color=BLEU_CALCUL, stroke_width=3)
          for k in range(5)]
    )
    return VGroup(vagues, coque, mat, foc, grande)


def cygne_dessine() -> VGroup:
    """Yarn: the ball of wool, three crossing strands, and the loose end."""
    # ⛔ Les aiguilles étaient deux traits POSÉS EN L'AIR au-dessus de la
    # pelote, sans rien pour les tenir. Retirées : une pelote se reconnait à ses
    # brins croisés et à son fil qui pend, pas à ce qu'on met autour.
    boule = Circle(radius=0.38, stroke_color=ROUGE_ERREUR, stroke_width=5)
    boule.set_fill(opacity=0)
    brins = VGroup(
        *[ArcBetweenPoints(np.array([-0.36, y, 0]), np.array([0.36, -y, 0]),
                           angle=a, stroke_color=ROUGE_ERREUR, stroke_width=2)
          for y, a in ((0.10, 1.0), (-0.10, -1.0), (0.24, 0.5))]
    )
    fil = VMobject(stroke_color=ROUGE_ERREUR, stroke_width=3)
    fil.set_fill(opacity=0)
    fil.start_new_path(np.array([0.34, -0.16, 0]))
    fil.add_cubic_bezier_curve_to(
        np.array([0.62, -0.22, 0]), np.array([0.54, -0.52, 0]),
        np.array([0.32, -0.48, 0]))
    return VGroup(boule, brins, fil)

def pyramide_dessine() -> VGroup:
    """A yolk: the round yellow centre, in the white of a cracked egg."""
    blanc = VMobject(stroke_color=WHITE, stroke_width=4)
    blanc.set_fill(opacity=0)
    blanc.start_new_path(np.array([-0.56, -0.06, 0]))
    blanc.add_cubic_bezier_curve_to(
        np.array([-0.62, 0.26, 0]), np.array([-0.22, 0.36, 0]),
        np.array([-0.02, 0.30, 0]))
    blanc.add_cubic_bezier_curve_to(
        np.array([0.22, 0.42, 0]), np.array([0.60, 0.22, 0]),
        np.array([0.54, -0.06, 0]))
    blanc.add_cubic_bezier_curve_to(
        np.array([0.62, -0.34, 0]), np.array([0.18, -0.44, 0]),
        np.array([-0.06, -0.36, 0]))
    blanc.add_cubic_bezier_curve_to(
        np.array([-0.34, -0.44, 0]), np.array([-0.58, -0.28, 0]),
        np.array([-0.56, -0.06, 0]))
    jaune = Circle(radius=0.21, stroke_color=JAUNE_TITRE, stroke_width=5)
    jaune.set_fill(opacity=0).move_to(np.array([-0.02, -0.02, 0]))
    reflet = ArcBetweenPoints(np.array([-0.14, 0.08, 0]), np.array([-0.02, 0.14, 0]),
                              angle=-0.9, stroke_color=WHITE, stroke_width=3)
    return VGroup(blanc, jaune, reflet)


def bicyclette_dessine() -> VGroup:
    """A yoghurt: the pot, its foil lid, and a spoon."""
    pot = VMobject(stroke_color=WHITE, stroke_width=5)
    pot.set_fill(opacity=0)
    pot.start_new_path(np.array([-0.30, 0.24, 0]))
    pot.add_line_to(np.array([-0.20, -0.40, 0]))
    pot.add_line_to(np.array([0.20, -0.40, 0]))
    pot.add_line_to(np.array([0.30, 0.24, 0]))
    pot.add_line_to(np.array([-0.30, 0.24, 0]))
    opercule = Ellipse(width=0.62, height=0.16, stroke_color=ROUGE_ERREUR,
                       stroke_width=4)
    opercule.set_fill(opacity=0).move_to(np.array([0.00, 0.24, 0]))
    languette = Polygon(
        np.array([0.26, 0.28, 0]), np.array([0.46, 0.42, 0]),
        np.array([0.34, 0.20, 0]),
        stroke_color=ROUGE_ERREUR, stroke_width=3).set_fill(opacity=0)
    bande = Line(np.array([-0.26, -0.08, 0]), np.array([0.26, -0.08, 0]),
                 stroke_color=BLEU_CALCUL, stroke_width=4)
    cuillere = VGroup(
        Line(np.array([0.38, -0.34, 0]), np.array([0.52, 0.18, 0]),
             stroke_color=GREY_B, stroke_width=4),
        Ellipse(width=0.14, height=0.20, stroke_color=GREY_B, stroke_width=3)
        .set_fill(opacity=0).move_to(np.array([0.55, 0.28, 0])),
    )
    return VGroup(cuillere, pot, bande, opercule, languette)


# ─── La voix ──────────────────────────────────────────────────────────────────
VOIX = Path(__file__).resolve().parents[3] / "public" / "sons" / "en-lettre-y"
DUREE = { "00-aujourdhui": 3.66, "01-ecoute": 3.99, "02-regarde": 3.03,
    "03-depart": 14.68, "04-encore": 4.15, "05-cherchons": 4.5,
    "05-y-comme": 2.08, "06-stylo": 1.77, "07-pyjama": 1.71, "08-cygne": 1.74,
    "09-pyramide": 1.77, "10-bicyclette": 1.95, "10-pareil": 5.16,
    "11-relance": 3.59, "12-va-sur": 3.56, "13-tout": 9.71,
    "14-bientot": 2.23,
}
CLIPS_MOTS = ["06-stylo", "07-pyjama", "08-cygne", "09-pyramide", "10-bicyclette"]


class _LetterYBase(Scene):
    vertical = False
    gaucher = False
    ...
    def construct(self):
        ...
        titre_mots = Text("y as in…", font_size=50 if not self.vertical else 38)
...
class LetterYRight(_LetterYBase):
    """16:9, droitier — conservée, mais on ne la rend plus."""


class LetterYLeft(_LetterYBase):
    gaucher = True


class LetterYPortraitRight(Portrait, _LetterYBase):
    """9:16, droitier — LE format qui est vu."""


class LetterYPortraitLeft(Portrait, _LetterYBase):
    gaucher = True
