...
// L'ANGLAIS DU JOUR — « 5 mots par jour », façon Duolingo mais SANS ses
// pièges : pas de cœurs, pas de gemmes, pas de série qui culpabilise, pas de
// pub, pas de paywall. On DÉCOUVRE le mot (avec le son), on le RECONNAÎT, puis
// on le REVOIT les jours suivants (répétition espacée). Chaque mot renvoie au
// coach pour en faire une phrase : on apprend en produisant, pas en tapant des
// cases (cf. « raisonner-en-creant-pas-avec-le-coach »).
//
// Source des mots : le COACH d'anglais (banques A1→B2), avec l'audio mp3 quand
// il existe. Tout se joue en local (localStorage) : marche pour 100 % des
// visiteurs, sans compte et sans base. Le suivi élève connecté viendra après.
...
import {
  ...
  NIVEAUX_REPERTOIRE,
  type MotRepertoire,
} from "@/lib/repertoire/anglais";
import { speakText } from "@/app/tutor-v4/ListenButton";

const STREAK_KEY = "anglais-du-jour-streak";
const LOG_KEY = "anglais-du-jour-log";
const NIVEAU_KEY = "anglais-du-jour-niveau";

type EntreeLog = { first: string; last: string; reviews: number };
type Log = Record<string, EntreeLog>;

type Etape =
  | { kind: "learn"; mot: MotRepertoire }
  | { kind: "quiz"; mot: MotRepertoire; revision: boolean };
...
// L'audio : le mp3 enregistré (accent juste) et, à défaut ou en cas d'échec, la
// voix anglaise du système. On commence par l'anglais justement parce que cette
// voix est presque toujours présente.
function jouer(mot: MotRepertoire) {
  if (mot.audio) {
    try {
      const a = new Audio(mot.audio);
      a.play().catch(() => speakText(mot.en, "en"));
      return;
    } catch {
      /* repli ci-dessous */
    }
...
export default function AnglaisDuJourClient() {
  ...
  const [motsJour, setMotsJour] = useState<MotRepertoire[]>([]);
  ...
  // Construit (ou reconstruit) la session pour un niveau donné.
  function construire(niv: string) {
    ...
    const jour = getMotsDuJour(now, 5, niv);
    const idsJour = new Set(jour.map((m) => m.id));
    const today = jourStr(now);

    // Les mots « à revoir » aujourd'hui (tous niveaux confondus, hors mots du
    // jour), les plus anciens d'abord, plafonnés à 3.
    const log = lireLog();
    const dus: { mot: MotRepertoire; last: string }[] = [];
    for (const [id, e] of Object.entries(log)) {
      if (idsJour.has(id)) continue;
      const mot = MOT_PAR_ID.get(id);
      if (!mot) continue;
  ...
  // Montage : niveau mémorisé + première session (côté client, la date ne doit
  // pas diverger entre serveur et navigateur).
  useEffect(() => {
  ...
  function repondre(choix: string, mot: MotRepertoire) {
    if (choisi) return;
    setChoisi(choix);
    if (choix === mot.en) setScore((s) => s + 1);
  }
      ...
      <div className="mx-auto max-w-xl">
        <div className="mb-4 text-center">
          <p className="text-sm font-black uppercase tracking-wide text-indigo-600">
            🇬🇧 L&apos;anglais du jour — 5 mots par jour
          </p>
        ...
        {/* Le choix du niveau (CECRL) — mémorisé d'une visite à l'autre. */}
        <div className="mb-4 flex flex-wrap justify-center gap-2">
          {NIVEAUX_REPERTOIRE.map((n) => (
            <button
              key={n.slug}
              type="button"
        ...
        {!etapes ? (
          ...
          <div className="rounded-3xl border border-fuchsia-200 bg-white p-6 text-center shadow-sm sm:p-8">
                ...
                >
                  <span className="flex items-baseline gap-2">
                    <button
                      type="button"
                      onClick={() => jouer(m)}
                      aria-label={`Écouter ${m.en}`}
                      className="text-indigo-500 hover:text-indigo-700"
                    >
                      🔊
                    </button>
                    <span className="font-black text-slate-800">{m.en}</span>
            ...
            <Link
              ...
              className="mt-5 block rounded-xl bg-indigo-600 px-6 py-3 text-base font-black text-white shadow-sm hover:bg-indigo-700"
            >
              ✍️ Fais une phrase avec un mot → le coach d&apos;anglais
            </Link>
          ...
          <Carte pas={pas} total={etapes.length} numQuiz={numQuiz} quizTotal={quizTotal} streak={streak}>
            ...
            <p className="mt-3 text-center text-4xl font-black text-slate-900">{etape.mot.en}</p>
...
function QuizCarte({
  ...
  etape: { kind: "quiz"; mot: MotRepertoire; revision: boolean };
  pas: number;
  total: number;
  numQuiz: number;
  quizTotal: number;
  streak: number;
  choisi: string | null;
  onEcouter: (mot: MotRepertoire) => void;
  onRepondre: (choix: string, mot: MotRepertoire) => void;
  onSuivant: () => void;
  dernier: boolean;
}) {
  const { mot, revision } = etape;
  const choix = useMemo(() => choixQuiz(mot), [mot]);
  ...
  return (
    <Carte pas={pas} total={total} numQuiz={numQuiz} quizTotal={quizTotal} streak={streak}>
      ...
      <p className="text-center text-lg font-bold text-slate-700">
        Comment dit-on <span className="font-black text-slate-900">« {mot.fr} »</span> en anglais ?
      </p>
        ...
        <div className="mt-4">
          <div
            className={`rounded-2xl p-3 text-center text-sm font-bold ${
              choisi === mot.en ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"
            }`}
          >
            {choisi === mot.en ? (
              <>✅ Oui&nbsp;! <b>{mot.en}</b> = {mot.fr}</>
            ) : (
              <>
                Presque&nbsp;! <b>{mot.en}</b> veut dire «&nbsp;{mot.fr}&nbsp;». On le reverra. 💛
              </>
            )}
          </div>
          <button
