...
/* ⛔ CETTE LISTE DÉCIDE DE CE QU'UN ÉLÈVE PEUT CLIQUER EN FRANÇAIS, et elle est
   ...
   ✅ LA 2de EST OUVERTE DEPUIS LE 18/08/2026, ET C'EST MESURÉ.
   `npx --yes tsx@4 scripts/verifier-demarrage.ts seconde francais complete`
   → **96 micros sur 96 ouvrent la ligne cliquée**, zéro repli silencieux.
   Le matin même, la même commande rendait 1 franche, 5 détournées et 90 qui
   levaient : la classe n'avait qu'UN item par micro, quand le mode complet en
   oppose deux. Les 96 seconds items ont été écrits dans la journée.
...
/* Le coach IA est adossé au référentiel Pix depuis le 16/08/2026 : deux
   ...
   L'échelle maison A1→C1 n'est plus proposée — son contenu reste en place le
   temps d'être reversé, et `normalizeClasse` renvoie proprement un ancien lien
   `?classe=a1` vers la première classe de la liste plutôt que de casser. */
const IA_CLASSES: Classe[] = ["pix-college", "pix-lycee"];
...
export default function CoachIA() {
  ...
  function handleClick(notionId: string, microId: string) {
    router.push(
      `/coach/serie?classe=${encodeURIComponent(classe)}&matiere=${encodeURIComponent(matiere)}&notion=${encodeURIComponent(notionId)}&microId=${encodeURIComponent(microId)}&${displayParamForClasse(classe)}`
    );
  }
